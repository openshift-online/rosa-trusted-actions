...
| `--namespace` | *(empty)* | Target namespace (omit for cluster-scoped resources) |
...
- **Cluster-scoped resources**: When `--namespace` is omitted, cluster-scoped resources (e.g. `namespaces`, `nodes`, `clusterroles`) are accessed directly without namespace scoping.
