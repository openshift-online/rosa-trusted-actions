...
Specialized agents for ROSA Trusted Actions Server development workflows.
...
- Test failures in CI
- Before creating PR
- After regenerating OpenAPI code
...
"Invoke test-agent on internal/handlers"
```
...
3. security-agent: Scan for secrets
...
Example: [lint-agent] [ERROR] internal/handlers/api.go:42: unreachable code
Example: [security-agent] [CRITICAL] .env:3: Hardcoded API key detected
```
